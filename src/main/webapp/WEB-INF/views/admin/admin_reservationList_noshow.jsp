...
<body id="htmlBody">
	...
	<div class="container" >
		...
		<div style="margin-top:3%;">
			<form name="searchHistByDateForm" id="searchHistByDateForm" method="post" action="">
				...
				<div class="col-lg-2">
					<button type="submit" onClick="noshowSearchFormSubmit()" class="pull-right btn btn-primary">검색</button>
				</div>
				...
				<ul class="nav nav-tabs">
			  		...
			  		<li id="tab3" class="active" onclick="noshowSearchFormSubmit()" ><a href="#" data-toggle="tab" aria-expanded="false">No-Show</a></li>
				</ul> <br>
				
				<!-- 정렬 -->
				<div class="col-xs-3">
					<select class="form-control"  name="sort" id="sort" onchange="noshowSearchFormSubmit()">
	  	...
	  	<div id="searchGeneralHistoryResultList" class="col-lg-12 table-responsive" style="margin-top:3%;">
				<table class="table table-hover text-center" style="text-align:center; font-size:100%">
					...
					<tbody>
						<c:forEach items="${noshowHistorySearchResult}" var="noshowHistorySearchResult">
							<tr>
								<td width="10%">${noshowHistorySearchResult.HST_DATE}</td>
								<td width="10%"><c:out value="${fn:substring(noshowHistorySearchResult.HST_START_TIME,0,5)}"/> ~
							<c:out value="${fn:substring(noshowHistorySearchResult.HST_END_TIME,0,5)}"/></td>
								<td width="10%">${noshowHistorySearchResult.CONF_NM}</td>
								<td width="20%">${noshowHistorySearchResult.HST_RSV_TITLE}</td>
								<td width="10%">${noshowHistorySearchResult.HST_RSV_MEM_NM}</td>
								<td width="15%">${noshowHistorySearchResult.HST_RSV_MEM_PN}</td>
								<td width="10%">${noshowHistorySearchResult.HST_REG_DATE}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
	...
	<%-- 첫 페이지로 이동 --%>
	<p align="center">
		<a href="/AdminReservCheckAndDelete/SearchNoshowHistory?page=1&sort=${requestScope.sort }&selectStartDate=${requestScope.selectStartDate }&selectEndDate=${requestScope.selectEndDate }">첫 페이지</a>

		<%-- 이전 페이지 그룹 처리 --%>
			<c:choose>
			<c:when test="${requestScope.pageBean.previousPageGroup }">
				<%-- 이전 페이지 그룹이 있다면 isPreviousPageGroup() 호출 --%>
				<a href="/AdminReservCheckAndDelete/SearchNoshowHistory?page=${requestScope.pageBean.beginPage - 1 }&sort=${requestScope.sort }&selectStartDate=${requestScope.selectStartDate }&selectEndDate=${requestScope.selectEndDate }">◀</a>
			</c:when>
			<c:otherwise>
			◀
		</c:otherwise>
		...
		<c:forEach begin="${requestScope.pageBean.beginPage }"
			...
			<c:choose>
				<c:when test="${requestScope.pageBean.page != page }">
					<!-- 현재 페이지가 아니라면 -->
					<a href="/AdminReservCheckAndDelete/SearchNoshowHistory?page=${page}&sort=${requestScope.sort }&selectStartDate=${requestScope.selectStartDate }&selectEndDate=${requestScope.selectEndDate }">${page }&nbsp;&nbsp;</a>
				</c:when>
				<c:otherwise>
				[${page }]  <%-- &nbsp;는 공백을 나타냄 --%>
				</c:otherwise>
		...
		<c:choose>
			<c:when test="${requestScope.pageBean.nextPageGroup }">
				<%-- isNextPageGroup() 호출 --%>
				<a
					href="/AdminReservCheckAndDelete/SearchNoshowHistory?page=${requestScope.pageBean.endPage + 1 }&sort=${requestScope.sort }&selectStartDate=${requestScope.selectStartDate }&selectEndDate=${requestScope.selectEndDate }">▶</a>
		...
		<!-- 마지막 페이지로 이동 -->
		<a href="/AdminReservCheckAndDelete/SearchNoshowHistory?page=${requestScope.pageBean.totalPage}&sort=${requestScope.sort }&selectStartDate=${requestScope.selectStartDate }&selectEndDate=${requestScope.selectEndDate }">마지막
			페이지</a>
	</p>
