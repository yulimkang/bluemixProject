...
<body id="htmlBody">
	...
	<div class="container">
		...
		<ul class="nav nav-tabs">
			...
			<div class="panel panel-default">
				<div class="panel-body">반복예약 검색 결과</div>
			</div>
	...
	<%-- 첫 페이지로 이동 --%>
	<p align="center">
		<a href="/Search/RepeatSearchPage?page=1&sort=${requestScope.sort }&selectSearchOpt=${requestScope.selectSearchOpt }&inputSearchCont=${requestScope.inputSearchCont }">첫 페이지</a>

		<%-- 이전 페이지 그룹 처리 --%>
		<c:choose>
			<c:when test="${requestScope.pageBean.previousPageGroup }">
				<%-- 이전 페이지 그룹이 있다면 isPreviousPageGroup() 호출 --%>
				<a href="/Search/RepeatSearchPage?page=${requestScope.pageBean.beginPage - 1 }&sort=${requestScope.sort }&selectSearchOpt=${requestScope.selectSearchOpt }&inputSearchCont=${requestScope.inputSearchCont }">◀</a>
			</c:when>
			<c:otherwise>
			◀
		</c:otherwise>
		...
		<c:forEach begin="${requestScope.pageBean.beginPage }"
			...
			<c:choose>
				<c:when test="${requestScope.pageBean.page != page }">
					<!-- 현재 페이지가 아니라면 -->
					<a href="/Search/RepeatSearchPage?page=${page}&sort=${requestScope.sort }&selectSearchOpt=${requestScope.selectSearchOpt }&inputSearchCont=${requestScope.inputSearchCont }">${page }&nbsp;&nbsp;</a>
				</c:when>
				<c:otherwise>
				[${page }]  <%-- &nbsp;는 공백을 나타냄 --%>
				</c:otherwise>
		...
		<c:choose>
			<c:when test="${requestScope.pageBean.nextPageGroup }">
				<%-- isNextPageGroup() 호출 --%>
				<a
					href="/Search/RepeatSearchPage?page=${requestScope.pageBean.endPage + 1 }&sort=${requestScope.sort }&selectSearchOpt=${requestScope.selectSearchOpt }&inputSearchCont=${requestScope.inputSearchCont }">▶</a>
		...
		<!-- 마지막 페이지로 이동 -->
		<a href="/Search/RepeatSearchPage?page=${requestScope.pageBean.totalPage}&sort=${requestScope.sort }&selectSearchOpt=${requestScope.selectSearchOpt }&inputSearchCont=${requestScope.inputSearchCont }">마지막
			페이지</a>
	</p>
 ...
 */
 ...
 /**
 * 작성자 : 최문정
 * 내용 : Form에서 입력받은 값 GeneralSearchPage로 전송(일반예약 내역 검색)
 */
